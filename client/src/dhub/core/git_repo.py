...
# Cap how long any single git invocation may run. Without this, a hanging or
# very slow remote (network glitch, server stall) blocks the CLI indefinitely.
...
def clone_repo(repo_url: str, ref: str | None = None) -> Path:
    ...
    try:
        if ref and _looks_like_sha(ref):
            ...
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=_GIT_TIMEOUT_SECONDS)
        ...
        shutil.rmtree(tmp_dir, ignore_errors=True)
        raise RuntimeError(
            f"git operation timed out after {_GIT_TIMEOUT_SECONDS}s — the remote may be unreachable or very slow"
        ) from None
