...
Your working directory is `.` (the current directory). You MUST:
...
model_bundle = {
    ...
    "draws": 500,        # Number of MCMC draws per chain
...
Before running Phase 2, check if your model is complex enough to warrant caution:
...
**If 2+ factors are "Complex" or any are "Very Complex", consider starting with fewer draws to test convergence before running the full fit.**

For example, first run with `draws=100, tune=500, chains=2` to check:
- Whether the sampler encounters divergences
- Whether R-hat values are trending toward convergence
- Approximate per-draw sampling speed to estimate total fitting time

**INTERPRETING RESULTS AND DECIDING WHAT TO DO:**

| Test Run Result | Action |
|-----------------|--------|
| Fast sampling, no divergences | Proceed with full MCMC fit (draws=500, tune=1500, chains=4) |
| Moderate speed, few divergences | Proceed with MCMC - normal for complex models |
| Slow sampling or many divergences | Consider simplifying model before full fit |
| Very slow (>1 sec/draw) with many params | Simplify model before fitting |

**WHEN TO SIMPLIFY MODEL:**

For models where the test run indicates very slow sampling or many divergences, consider simplification:
...
-> Run test fit with draws=100, tune=500, chains=2
-> Observe: 0.8 sec/draw, some divergences

Decision: Slow sampling -> Simplify model
-> Re-run Phase 1 with hierarchical priors
-> Re-test: 0.2 sec/draw, no divergences
...
### Phase 2: Fit (Run the fitting CLI)

**MANDATORY: Use the `mmm_lib.fit_model_cli` module for MCMC fitting. NEVER write your own fitting script.**

```bash
python -m mmm_lib.fit_model_cli model_to_fit.pkl
```

Output is saved to `fitted_model.nc` in the current directory by default.

**Sampling takes time: MCMC sampling can take 10 minutes to several hours depending on model complexity.** This is NORMAL. Do NOT stop or interrupt. Factors affecting runtime:
...
### Phase 3: Analyze (Run the analysis CLI)

**MANDATORY: Use the `mmm_lib.analyze_model_cli` module for post-fit analysis. NEVER write your own analysis script.**

```bash
python -m mmm_lib.analyze_model_cli fitted_model.nc
```

If the orchestrator indicated that channels are not in comparable monetary units (e.g., mix of impressions, clicks, USD), pass `--no-roas`:

```bash
python -m mmm_lib.analyze_model_cli fitted_model.nc --no-roas
```

The CLI runs all analysis automatically (diagnostics, posterior predictive, contributions, ROAS, saturation, sensitivity) and returns the full output directly. Read the returned text to extract key metrics, then write `summary.md`.

**DO NOT** write `phase3_analyze.py` or any other analysis script. **DO NOT** manually call `mmm.summary.roas()`, `check_convergence()`, `sample_posterior_predictive()`, or any other analysis functions. The CLI does all of this for you.

### Write comprehensive `summary.md`

After the analysis CLI completes, write a detailed `summary.md` file with ALL findings from the tool output.
...
# Fit model (Phase 2 - the fit CLI does this, DO NOT call fit() directly)
# mmm.fit(X=X, y=y, nuts_sampler="numpyro")  # Handled by fit_model_cli
```
...
- Can use the fit CLI (it supports both single and multi-dimensional MMM)
...
**Note:** The `mmm_lib.fit_model_cli` module handles Phase 2 (MCMC fitting). You don't need to call `fit_mmm_model` directly.
...
| **2. Fit** | Run MCMC sampling via fit_model_cli | `fitted_model.nc` |
...
### PHASE 2: Fit Model Using fit_model_cli

**Use the `mmm_lib.fit_model_cli` module to run MCMC sampling.** This separates the computationally expensive fitting from the preparation and analysis phases.
...
| `chains` | 2-8 | More chains = better ESS and diagnostics. Match to available CPU cores. |
| `target_accept` | 0.8-0.95 | Higher values reduce divergences but slow sampling. |

**Recommended strategy:** Use higher tune (1500) with adequate chains and draws:
- `tune=1500, draws=500, chains=4` -> 4 x 2000 = 8,000 total samples
- Better adaptation from longer tuning
- Good ESS from multiple chains
- Scale chains up if more CPU cores are available

**NEVER set random_seed=42 or any fixed seed!** You are one of several parallel instances exploring different model configurations. Using the same seed across instances defeats the purpose of parallel exploration. Let the sampler use random initialization.

The fit_model_cli module:
1. Reads the model bundle from `model_to_fit.pkl` (containing mmm, X, y, and sampling params)
2. Runs MCMC fitting with numpyro backend
3. Saves the fitted model to `fitted_model.nc` using canonical `mmm.save()`
4. Returns timing info and model type

**Check that `fitted_model.nc` was created after the CLI completes.**

---

### PHASE 3: Analyze Fitted Model (USE THE `analyze_model_cli` — DO NOT WRITE A SCRIPT)

**MANDATORY: Use the analyze_model_cli module. NEVER write your own analysis script (no `phase3_analyze.py`).**
...
The CLI returns the full analysis output directly. Read the returned text to extract key metrics.

**If the `analyze_model_cli` fails:** Report the error in your `summary.md`. If the error is due to a model configuration that doesn't fit the CLI's assumptions (e.g., unusual model structure), you may write custom analysis code — but read `mmm_lib/analyze_model.py` first to understand the patterns. If the error looks like a bug, report it and STOP.

**After the CLI completes, check convergence from the output:**
...
Even for non-converged models, ALWAYS run the `analyze_model_cli` and include ROAS/contribution sections below. The results carry more uncertainty but help diagnose what went wrong and give the orchestrator evidence to work with.
...
- If you believe X% of sales come from marketing, intercept_mu ~ y_scaled_mean x (1 - X/100)
...
2. **FIX THE CODE** in your phase script (`phase1_prepare.py`)
3. **RE-RUN** the script

**Retry Limit:** Maximum 10 attempts to fix any single error.

**Goal:** All phase scripts must run without errors. Use the `mmm_lib.fit_model_cli` module for Phase 2.
...
- [ ] `analyze_model_cli` run (even if not converged — results help diagnosis)
- [ ] Fitted model saved
