...
**Use Python to inspect each data file:**

```bash
python -c "from mmm_lib import load_csv_or_parquet_from_file, inspect_dataframe_basic; df = load_csv_or_parquet_from_file('data/your_file.csv'); print(inspect_dataframe_basic(df))"
```
...
df = load_csv_or_parquet_from_file("data/your_file.csv")
...
**Use the Agent tool to spawn 2 data-preparer sub-agents with different prompts.**

The Agent SDK runner handles parallel execution. Spawn each sub-agent with the same core prompt but allow them to take independent approaches.
...
After both complete, compare their approaches and consolidate.
...
**If all models agree** on column mappings, proceed with that configuration.
...
**Use the Agent tool to spawn sub-agents for each modeling plan.** The Agent SDK runner handles parallel execution. Create one sub-agent per modeling plan, each with a prompt specifying that plan's configuration.

**You can combine data preparation variations with modeling variations** — give each sub-agent a prompt describing its specific plan.

**DO NOT specify sampling parameters in modeler prompts!**
...
4. If converged: run analysis, compute ROAS and contributions
5. If NOT converged: run analysis anyway (for diagnosis), write diagnosis
6. Write a `summary.md` with results (converged) or diagnosis (not converged)

After all modelers complete, compare and consolidate their approaches.
...
#### CRITICAL: Adding Caveats Does NOT Fix Conflicts
...
**STOP. DO NOT CALL THE ANALYST AGENT. DO NOT MAKE BUDGET RECOMMENDATIONS.**
...
**STOP AND VERIFY:** Before proceeding, confirm:
...
**Core rule: NEVER recommend a budget reallocation you have not simulated.** Every specific recommendation in the report must be backed by a budget optimization run. If your recommendation would exceed the bounds of your simulated scenarios, calculate the `risk_pct` needed to reach that allocation, run the optimizer at that level, and report the optimizer's output — not your own estimate.
...
```bash
python -m mmm_lib.optimize_budget_cli best_model.nc --risk-pct 0.05 --output-dir budget_5pct/
```

```bash
python -m mmm_lib.optimize_budget_cli best_model.nc --risk-pct 0.10 --output-dir budget_10pct/
```

```bash
python -m mmm_lib.optimize_budget_cli best_model.nc --risk-pct 0.20 --output-dir budget_20pct/
```

```bash
python -m mmm_lib.optimize_budget_cli best_model.nc --risk-pct 0.50 --output-dir budget_50pct/
```

```bash
python -m mmm_lib.optimize_budget_cli best_model.nc --risk-pct 1.0 --output-dir budget_100pct/
```

```bash
python -m mmm_lib.optimize_budget_cli best_model.nc --risk-pct 3.0 --output-dir budget_300pct/
```
...
**Model analysis CLI** (use instead of writing custom analysis scripts):
```bash
python -m mmm_lib.analyze_model_cli fitted_model.nc
```

**Model fitting CLI** (use for fitting serialized model specs):
```bash
python -m mmm_lib.fit_model_cli model_to_fit.pkl
```

**Budget optimization CLI** (use for running budget scenarios):
```bash
python -m mmm_lib.optimize_budget_cli best_model.nc --risk-pct 0.10 --output-dir budget_10pct/
```
...
- Per-modeler `summary.md` - Results from each modeler
...
- **How to detect:** Check the data-preparer summary for channel units. If channels include non-monetary metrics (impressions, clicks, views), set ROAS as not applicable
...
When ROAS is not applicable, pass the flag to the CLI tools:
...
```bash
python -m mmm_lib.optimize_budget_cli best_model.nc --risk-pct 0.25 --no-roas --output-dir budget_25pct/
```
...
If you need analysis beyond what the `analyze_model_cli` provides, you can read the analysis module source to understand how things are done, then write your own scripts:

```python
import mmm_lib.analyze_model as analyze_module
```

Read this module to understand the available functions, their signatures, and how they work internally. Then write custom Python scripts that build on or adapt these patterns for your specific needs.
