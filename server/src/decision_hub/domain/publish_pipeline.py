"""Publish pipeline — pure business logic for skill publishing.

Functions in this module handle manifest parsing, gauntlet pipeline,
classification, quarantine, and eval triggering.  They raise domain
exceptions (see ``domain.exceptions``) instead of HTTP exceptions,
keeping framework concerns out of the domain layer.

The API layer (``api.registry_service``) wraps these with thin HTTP
translators that convert domain exceptions to ``HTTPException``.
"""
...
def require_org_membership(
    ...
    *,
    ...
    Raises:
        OrgNotFoundError: Organisation not found.
        NotOrgMemberError: User is not a member.
        AdminRequiredError: admin_only=True but user lacks admin/owner role.
    """
    ...
    if member is None:
        raise NotOrgMemberError("You are not a member of this organisation")
    if admin_only and member.role not in ("owner", "admin"):
        raise AdminRequiredError("Only org owners and admins can perform this action")
    return org
...
def parse_manifest_from_content(
    ...
    Raises:
        ManifestParseError: If the manifest is malformed.
    """
    ...
    try:
        ...
        raise ManifestParseError(f"SKILL.md manifest is malformed: {exc}") from exc
    finally:
        tmp_path.unlink()
...
def quarantine_rejected_skill(
    ...
    *,
    ...
    """Upload rejected zip to quarantine, log the rejection, and raise.

    Thin wrapper around ``quarantine_and_log_rejection`` that raises a
    ``GauntletRejectionError`` afterwards.
    """
    ...
    raise GauntletRejectionError(summary=report.summary)
...
def try_parse_assessment_cases(file_bytes: bytes):
    ...
    Raises:
        EvalCaseParseError: If eval files exist but are malformed.
    """
    ...
    try:
        ...
        raise EvalCaseParseError(f"Eval case files are malformed: {exc}") from exc
...
def maybe_trigger_agent_assessment(
    ...
    Raises:
        EvalConfigError: Config declared but no case files found.
    """
    if eval_config and not eval_cases:
        raise EvalConfigError("Assessment config declared in manifest but no case files found in evals/")
