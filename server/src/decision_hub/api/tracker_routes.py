...
# Tracker CRUD is authenticated but previously had no rate limit. Reuse the
# publish budget since creating a tracker triggers tracker polling which in
# turn exercises the publish path — same ceiling is appropriate.
router = APIRouter(
    prefix="/v1/trackers",
    tags=["trackers"],
    dependencies=[rate_limit("publish")],
)
