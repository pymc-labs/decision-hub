...
# Shared rate-limit dependency for /auth/* endpoints. See rate_limit.py.
_rl_auth = rate_limit("auth")
...
@router.post("/github/code", response_model=DeviceCodeResponseSchema, dependencies=[Depends(_rl_auth)])
...
@router.post("/github/token", response_model=TokenResponse, dependencies=[Depends(_rl_auth)])
async def exchange_token(
    ...
    # Commit so newly created orgs are visible to the separate transactions
