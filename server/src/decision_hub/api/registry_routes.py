...
# Rate-limit dependencies for each endpoint. Each one reads the matching
# settings.<name>_rate_{limit,window} fields and lazily attaches a per-name
# RateLimiter to app.state, so all routes that share a name share a bucket.
...
def get_registry_stats(
    ...
    Hit on every homepage load — recomputing the COUNT/SUM aggregates per
    request was the largest fixed cost on that path.  Now served from the
    in-memory TTLCache with the configurable ``cache_ttl_stats`` window.
    """
    ttl = settings.cache_ttl_stats
    if ttl:
        response.headers["Cache-Control"] = f"public, max-age={ttl}"
        cached = cache.get("registry_stats")
        if cached is not None:
            return cached
    result = fetch_registry_stats(conn)
    if ttl:
        cache.set("registry_stats", result, ttl=ttl)
    return result
