...
# Rate-limit dependencies. Each name is registered in
# ``decision_hub.api.rate_limit._RATE_LIMIT_NAMES`` and backed by a
# ``<name>_rate_limit`` / ``<name>_rate_window`` pair on Settings. The
# dependency objects are created at module-import time and reused across
# every request, so per-request overhead is a single dict lookup.
_rl_list_skills = rate_limit("list_skills")
_rl_resolve = rate_limit("resolve")
_rl_similar_skills = rate_limit("similar_skills")
_rl_download = rate_limit("download")
_rl_audit_log = rate_limit("audit_log")
_rl_scan_report = rate_limit("scan_report")
_rl_publish = rate_limit("publish")
...
@router.post("/publish", response_model=PublishResponse, status_code=201, dependencies=[Depends(_rl_publish)])
def publish_skill(
    metadata: str = Form(...),
...
@public_router.get(
    ...
    dependencies=[Depends(_rl_list_skills)],
)
def list_skills(
...
@public_router.get(
    ...
    dependencies=[Depends(_rl_similar_skills)],
)
...
@public_router.get(
    ...
    dependencies=[Depends(_rl_resolve)],
)
def resolve_skill(
    org_slug: str,
    skill_name: str,
...
@public_router.get(
    ...
    dependencies=[Depends(_rl_audit_log)],
)
def get_audit_log(
    org_slug: str,
    skill_name: str,
...
@public_router.get(
    ...
    dependencies=[Depends(_rl_scan_report)],
)
def get_scan_report(
    org_slug: str,
    skill_name: str,
