...
# Rate-limit dependencies — built from a single factory so each one is a
# one-liner declaring (name, max-attr, window-attr).  The factory caches the
# RateLimiter instance lazily on app.state on first use; see
# ``make_rate_limit_dep`` in api.rate_limit for details.
