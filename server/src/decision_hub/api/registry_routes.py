...
@router.post("/publish", response_model=PublishResponse, status_code=201, dependencies=[Depends(rate_limit("publish"))])
def publish_skill(
    # Bound the metadata blob so a malicious client cannot burn memory on
    # JSON parsing or wedge the log pipeline with a multi-MB payload. The
    # real body is ~1 KB (org_slug, skill_name, version, visibility,
    # source_repo_url, manifest_path); 4 KB leaves comfortable headroom.
    metadata: str = Form(..., max_length=4096),
...
@public_router.get(
    ...
    dependencies=[Depends(rate_limit("list_skills"))],
)
def list_skills(
...
@public_router.get(
    ...
    dependencies=[Depends(rate_limit("similar_skills"))],
)
...
@public_router.get(
    ...
    dependencies=[Depends(rate_limit("resolve"))],
)
def resolve_skill(
    org_slug: str,
    skill_name: str,
...
@public_router.get(
    ...
    dependencies=[Depends(rate_limit("audit_log"))],
)
def get_audit_log(
    org_slug: str,
    skill_name: str,
...
@public_router.get(
    ...
    dependencies=[Depends(rate_limit("scan_report"))],
)
def get_scan_report(
    org_slug: str,
    skill_name: str,
