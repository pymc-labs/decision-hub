...
# Per-endpoint rate limiters. ``make_rate_limit_dep("foo")`` reads
# ``settings.foo_rate_limit`` / ``settings.foo_rate_window`` lazily on the
# first request and stores the limiter under ``app.state.rate_limiters``.
...
def _check_zombie(conn: Connection, run) -> str:
    ...
    Mutates the passed-in ``run`` dataclass in place when it marks the
    run as failed, so callers don't need to re-fetch from the database
    just to surface the new status to the user.
    """
    ...
    # Mirror the DB write onto the in-memory run so the caller sees the
    # same view it would have seen after a re-read, without the round-trip.
    # ``run`` is a frozen dataclass — use object.__setattr__ to bypass.
    object.__setattr__(run, "status", "failed")
    object.__setattr__(run, "error_message", error_message)
    object.__setattr__(run, "completed_at", completed_at)
    return "failed"
...
def get_eval_run(
    ...
    # _check_zombie mutates `run` in place when it marks the run failed,
