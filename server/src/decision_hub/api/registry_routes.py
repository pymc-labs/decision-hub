...
# Rate-limit dependencies are built by ``make_rate_limit_dep`` so each
# bucket (list/resolve/publish/...) shares one lazily-constructed
# RateLimiter per container. See decision_hub.api.rate_limit.
...
def download_skill(
    ...
    # ``org_slug`` and ``skill_name`` are user-controlled; never inline raw
    # into a header. RFC 6266 ``filename*=UTF-8''<pct-encoded>`` is the
    # canonical fix and is understood by every modern browser.
