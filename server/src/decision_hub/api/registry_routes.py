...
# Per-endpoint rate-limit dependencies.  Each call returns a FastAPI
# dependency that lazily binds a ``RateLimiter`` to ``app.state`` so the
# limiter is shared across requests within a container.  See
# ``decision_hub.api.rate_limit.make_rate_limit_dep`` for the factory.
...
def list_eval_runs(
    ...
    The current user's ID is always pushed into the query so this endpoint
    can never leak another user's runs even if a misconfigured query helper
