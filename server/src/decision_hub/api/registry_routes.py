...
# Each dependency is lazily attached to app.state on first request via the
# shared factory in ``rate_limit.py``; see ``rate_limit_dependency`` for the
# thread-safe init and X-Forwarded-For handling that replaced the previous
# copy-pasted ``_enforce_*`` helpers.
