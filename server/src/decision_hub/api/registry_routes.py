...
# Per-endpoint rate limiters. Each dependency is defined once at module scope
# so FastAPI's dependency cache treats it as a single dependency; the shared
# sliding-window state lives on ``app.state._{name}_rate_limiter``.
