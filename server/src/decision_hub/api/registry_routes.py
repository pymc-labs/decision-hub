...
# Per-endpoint rate limiters. Each ``rate_limit_dep`` call produces a
# FastAPI dependency that lazily instantiates a ``RateLimiter`` on
# ``app.state`` so the sliding-window state is shared across requests
# to the same endpoint but isolated between endpoints.
