...
# Per-endpoint rate limiters. Each `make_rate_limit_dependency(name)` binds
# to `settings.<name>_rate_limit` / `settings.<name>_rate_window` and lazily
# stores the shared `RateLimiter` on `app.state._<name>_rate_limiter` on
# first use.
...
def _check_zombie(conn: Connection, run: EvalRun) -> EvalRun:
    """Check if a running eval has a stale heartbeat and mark it failed.

    When the heartbeat is older than ``_STALE_HEARTBEAT_SECONDS`` the DB
    row is updated to ``status='failed'`` with an explanatory
    ``error_message`` and a fresh ``completed_at``; the returned ``EvalRun``
    reflects those changes so callers can respond without re-reading. When
    no zombie is detected the input ``run`` is returned unchanged.
    """
