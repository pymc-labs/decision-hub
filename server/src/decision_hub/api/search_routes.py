...
# Rate-limit the ``/v1/ask`` endpoint. Uses the shared factory so its lazy
# init and cross-thread safety match every other public route.
_enforce_search_rate_limit = rate_limit_dependency(
    "_search_rate_limiter",
    lambda s: (s.search_rate_limit, s.search_rate_window),
)
