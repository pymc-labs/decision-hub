...
# Shared rate-limit dependency for /ask (GET and POST). Built once at module
# import; per-request cost is a single dict lookup against
# ``app.state.rate_limiters``.
_rl_search = rate_limit("search")
...
@router.get(
    ...
    dependencies=[Depends(_rl_search)],
)
def ask_skills(
