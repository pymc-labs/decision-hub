...
- **Keep the ask pipeline in sync with skill metadata**: When adding new columns to the `skills` table or the hybrid search query (`search_skills_hybrid` in `database.py`), propagate the field through the full ask pipeline: (1) `SkillIndexEntry` model in `models.py`, (2) `build_index_entry()` and `serialize_index()` in `domain/search.py` so the LLM sees it, (3) `AskSkillRef` response model in `search_routes.py` and both enrichment paths (main + fallback), (4) the frontend `AskSkillRef` type in `types/api.ts` and `AskModal.tsx`. The LLM can only answer questions about metadata it receives in the JSONL index
