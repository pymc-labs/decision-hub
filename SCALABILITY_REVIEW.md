...
- **Token limit**: At ~100 bytes per JSONL line, 200k skills = ~20MB of text = millions of tokens. This exceeds Gemini's context window (1M tokens for Gemini 1.5 Pro, far less for Gemini Flash). The API call will fail outright.
- **Latency**: Even if it fit, processing millions of tokens would take 30+ seconds per query.
- **Cost**: Every search query pays for millions of input tokens.
...
|----------|-------|-----------------|
| **P0** | LLM search sends full index to Gemini | **Completely broken** — exceeds token limits |
